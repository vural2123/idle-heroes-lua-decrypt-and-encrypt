...
function frdarena.getHeroes()
	local all = {}
	if frdarena.team then
		for _, v in pairs(frdarena.team.mbrs) do
			if v.camp then
				for i, u in pairs(v.camp) do
					if not u.pos or u.pos <= 6 then
						all[#all + 1] = u
					end
				end
			end
...
function frdarena.getPets()
	local all = {}
	if frdarena.team then
		for _, v in pairs(frdarena.team.mbrs) do
			if v.camp then
				for i, u in pairs(v.camp) do
					if u.pos and u.pos == 7 then
						all[#all + 1] = u
					end
				end
			end
...
function frdarena.getHero(hid)
	if frdarena.team then
		for _, v in pairs(frdarena.team.mbrs) do
			if v.camp then
				for i, u in pairs(v.camp) do
					if (not u.pos or u.pos <= 6) and u.hid == hid then
						return u
					end
				end
			end
...
function onFrdarena(data)
	...
	if not data.invited or data.invited <= 1 then return end
