...
function ui.create(seasonranks)
    ...
    local function onUpdate(ticks)
        if friendboss.ccd then
            cd = math.max(0, friendboss.ccd + friendboss.pull_time - os.time())
            if cd > 0 then
                ...
                friendboss.ccd = friendboss.ccd + 7*24*3600 
                local timeLab = string.format("%02d:%02d:%02d",math.floor(cd/3600),math.floor((cd%3600)/60),math.floor(cd%60))
                showTimeLab:setString(timeLab)
	...
	local cfgassist = require ("config.seasonrank" .. seasonranks.seasonid)
