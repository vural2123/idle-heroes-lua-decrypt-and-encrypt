...
function ui.create(id)
    ...
    img.unload(img.packedOthers.ui_activity_crushing_space2_cn)
    if i18n.getCurrentLanguage() == kLanguageChinese 
        or i18n.getCurrentLanguage() == kLanguageChineseTW then
        img.load(img.packedOthers.ui_activity_crushing_space2_cn)
    else
        img.load(img.packedOthers.ui_activity_crushing_space2)
    end
    local banner = img.createUISprite(img.ui.activity_crush_board2)
    ...
    btn_rank:registerScriptTapHandler(function()
        ...
        layer:getParent():getParent():addChild((require"ui.activity.crushbossrank").create(2), 1000)
    end)
    ...
    btn_reward:registerScriptTapHandler(function()
        ...
        net:bboss_syn(gParams, function(__data)
            ...
            layer:getParent():getParent():addChild((require"ui.activity.crushreward").create(2, __data.id), 1000)
        end)
    end)
    ...
    btnInfo:registerScriptTapHandler(function()
        audio.play(audio.button)
        layer:getParent():getParent():addChild(require("ui.help").create(i18n.global.broken_space_help2.string, i18n.global.help_title.string), 1000)
    end)
    ...
    local function refreshTick(num)
        lbl_tick:setString(tick_num+num)
    end
    ...
    battleBtn:registerScriptTapHandler(function()
        ...
        audio.play(audio.button) 
        layer:getParent():getParent():addChild(crushbossfight.create(2), 1001) 
    end)
    local act_st = activityData.getStatusById(IDS.CRUSHING_SPACE_2.ID)
