...
function ui.create(callback)
    ...
    -- ticket icon
    local icon_ticket = img.createItem(ITEM_ID_BROKEN)--img.createUISprite(img.ui.casino_chip)
    ...
    btn_buy:registerScriptTapHandler(function()
        ...
        netClient:bboss_buy(params, function(__data)
            cclog("buy_ticket callback.")
            tbl2string(__data)
            delWaitNet()
            if __data.status == -2 then
                showToast(i18n.global.buy_actticket_limit.string)
                btn_buy:setEnabled(true)
                return
            end
            ...
            bagdata.items.add({ id = ITEM_ID_BROKEN, num = buyCount })
            callback(buyCount)
            btn_buy:setEnabled(true)
