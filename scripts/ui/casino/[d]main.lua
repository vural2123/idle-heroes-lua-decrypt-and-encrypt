...
function ui.create(uiParams)
    ...
    shop_bg:setPosition(scalep(960+33, 576))
    ...
    --particle_shop:setStartSize(particle_scale * (particle_shop:getStartSize()))
    --particle_shop:setStartSizeVar(particle_scale * particle_shop:getStartSizeVar())
    --particle_shop:setEndSize(particle_scale * particle_shop:getEndSize())
    --particle_shop:setEndSizeVar(particle_scale * particle_shop:getEndSizeVar())
    ...
    ---- base
    --local base = img.createUISprite(img.ui.casino_base)
    --base:setAnchorPoint(CCPoint(0.5, 0))
    --base:setPosition(CCPoint(bg_w/2-12, 0))
    --bg:addChild(base)

    ---- mm
    --local mm = img.createUISprite(img.ui.casino_mm)
    --mm:setAnchorPoint(CCPoint(0, 0))
    --mm:setPosition(CCPoint(0, 0))
    --bg:addChild(mm)

    ---- msg_bg
    --local msg_bg = img.createUISprite(img.ui.casino_msg_bg)
    --msg_bg:setAnchorPoint(CCPoint(1, 0.5))
    --msg_bg:setPosition(CCPoint(bg_w-10, bg_h/2-5))
    --bg:addChild(msg_bg)

    ---- ring
    --local ring = img.createUISprite(img.ui.casino_ring)
    --ring:setPosition(CCPoint(293, 283))
    --base:addChild(ring)
    --local ring_w = ring:getContentSize().width
    --local ring_h = ring:getContentSize().height

    ---- c2
    --local c2 = img.createUISprite(img.ui.casino_circle2)
    --c2:setPosition(CCPoint(ring_w/2+2, ring_h/2))
    --ring:addChild(c2)
    --local c2_w = c2:getContentSize().width
    --local c2_h = c2:getContentSize().height
    --local cx = c2_w/2
    --local cy = c2_h/2
    --local pr = 86
    --local pointer = img.createUISprite(img.ui.casino_pointer)
    --pointer:setAnchorPoint(CCPoint(0.5 , 0))
    --pointer:setPosition(CCPoint(cx, cy+pr))
    --c2:addChild(pointer, 2)
    ---- c1
    --local c1 = img.createUISprite(img.ui.casino_circle1)
    --c1:setPosition(CCPoint(c2_w/2, c2_h/2))
    --c2:addChild(c1, 3)
    ...
    cd_container:setPosition(CCPoint(480, 576-45))
    ...
    btn_log:setPosition(CCPoint(960-47, 409))
    ...
    local function onemore_1draw()
        ...
        casinodata.draw(params, function(__data)
            ...
            ani_1draw:addObject(CCCallFunc:create(function()
                arrow:runAction(CCEaseIn:create(CCRotateTo:create(2.4 / SPEED_MULT, 360*8), 2))
                ani_duchang:playAnimation("spin_start", 1)
                ani_duchang:appendNextAnimation("spin_end", 1, 0)
            end))
            ani_1draw:addObject(CCDelayTime:create(2.4 / SPEED_MULT))
            ani_1draw:addObject(CCCallFunc:create(function()
                local toAngle = 22.5 +(__data.ids[1]-1)*45
                arrow:runAction(CCEaseOut:create(CCRotateTo:create(2 / SPEED_MULT, 360*3+toAngle), 2))
            end))
            ani_1draw:addObject(CCDelayTime:create(2 / SPEED_MULT))
            ani_1draw:addObject(CCCallFunc:create(function()
                if __data.bag then
                    bagdata.addRewards(__data.bag)
                    layer:addChild(rewards.showReward(casinodata.ids2Pbbag(__data.ids), {count=1, callback=onemore_1draw}, false, SPEED_MULT), 1000)
                end
    ...
    local function onemore_10draw()
        ...
        casinodata.draw(params, function(__data)
            ...
            ani_10draw:addObject(CCCallFunc:create(function()
                arrow:runAction(CCEaseIn:create(CCRotateTo:create(4 / SPEED_MULT, 360*12), 2))
                ani_duchang:playAnimation("spin_start2", -1)
            end))
            ani_10draw:addObject(CCDelayTime:create(3 / SPEED_MULT))
            --ani_10draw:addObject(CCCallFunc:create(function()
            ...
            ani_10draw:addObject(CCCallFunc:create(function()
                if __data.bag then
                    bagdata.addRewards(__data.bag)
                    layer:addChild(rewards.showReward(casinodata.ids2Pbbag(__data.ids), {count=8, callback=onemore_10draw}, false, SPEED_MULT), 1000)
                end
                if __data.lucky_coin then
                    bagdata.items.add({id=ITEM_ID_LUCKY_COIN, num=__data.lucky_coin})
                end
