...
local function createItem(__data, round, videoId, orgData)
	...
	local leftBg = createItemDetail(round, __data.wins[round] == true, true, __data.atk)
	leftBg:setAnchorPoint(0, 0.5)
	leftBg:setPosition(0, height * 0.5)
	container:addChild(leftBg)

	local rightBg = createItemDetail(round, __data.wins[round] == false, false, __data.def)
    ...
    btn_vide:registerScriptTapHandler(function()
		...
		require("fight.helper.ccamp").processCamp(newData, nil, 2)
...
function ui.create(__data, videoId, orgData)
    ...
    local itemAry = {}
    for i = 1, 3 do
    	local item = createItem(__data, i, videoId, orgData)
    	if not item then
    		break
    	end
