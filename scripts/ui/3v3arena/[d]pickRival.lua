...
function ui.create()
    ...
    local showPower = lbl.createFont2(20, arena3v3Data.power)
    ...
    local function loadRivals(Rivals)
        ...
        for i, v in ipairs(Rivals) do
            ...
            playerHead:registerScriptTapHandler(function()
                audio.play(audio.button)
                layer:addChild(require("ui.3v3arena.player").create(v), 100)
            end)
            ...
            btnBattle:registerScriptTapHandler(function() 
                ...
                if havTicket >= ticketCost then
                    layer:addChild(require("ui.3v3arena.select").create({type = "3v3arenaAtk", info = v, cost = ticketCost}))
                else
    ...
    local function onRefresh()
        ...
        if #Rivals <= 0 then
            ...
            net:pvp_refresh(params, function(__data)
                ...
                tbl2string(__data)
                arena3v3Data.rivals = __data.rivals
                Rivals = arena3v3Data.refresh()
                loadRivals(Rivals)
            end)
        end
        loadRivals(Rivals)
