...
function ui.create(star)
    local layer = CCLayer:create()

    --local status = activitylimitData.getStatusById(IDS.FIRST_PAY.ID)
    --if not status then return end  -- error

    img.unload(img.packedOthers.ui_limit_summon)
    img.unload(img.packedOthers.ui_limit_summon_cn)
    if i18n.getCurrentLanguage() == kLanguageChinese 
        or i18n.getCurrentLanguage() == kLanguageChineseTW then
        img.load(img.packedOthers.ui_limit_summon_cn)
    else
        img.load(img.packedOthers.ui_limit_summon)
    end

    local board = img.createUISprite(img.ui.limit_summon_gift)
    ...
    if i18n.getCurrentLanguage() == kLanguageChinese then
          lable = img.createUISprite("limit_summon_label_cn.png")  
    elseif i18n.getCurrentLanguage() == kLanguageChineseTW then
          lable = img.createUISprite("limit_summon_label_tw.png")  
    elseif i18n.getCurrentLanguage() == kLanguageRussian then
          lable = img.createUISprite("limit_summon_label_ru.png")  
    elseif i18n.getCurrentLanguage() == kLanguageJapanese then
          lable = img.createUISprite("limit_summon_label_jp.png")  
    elseif i18n.getCurrentLanguage() == kLanguageKorean then
          lable = img.createUISprite("limit_summon_label_kr.png")  
    elseif i18n.getCurrentLanguage() == kLanguageSpanish then
          lable = img.createUISprite("limit_summon_label_sp.png")  
    elseif i18n.getCurrentLanguage() == kLanguagePortuguese then
          lable = img.createUISprite("limit_summon_label_pt.png")  
    else
          lable = img.createUISprite("limit_summon_label.png")  
    end
    ...
    local storeID
    local ids
    if star == cfglimitgift[IDS.SUMMON_4.ID].parameter then
        rewards = cfglimitgift[IDS.SUMMON_4.ID].rewards
        storeID = cfglimitgift[IDS.SUMMON_4.ID].storeId
        ids = IDS.SUMMON_4.ID
    elseif star == cfglimitgift[IDS.SUMMON_5.ID].parameter then
        rewards = cfglimitgift[IDS.SUMMON_5.ID].rewards
        storeID = cfglimitgift[IDS.SUMMON_5.ID].storeId
        ids = IDS.SUMMON_5.ID
    end
