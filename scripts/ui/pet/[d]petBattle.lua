...
function ui.create(layer,petCallBack)
    ...
    --ui.widget.battle_Spine:setScale(view.minScale)
...
function ui.findNum(num)
    for k,v in pairs(petNetData) do
        if v.isSele ~= nil and num ==  v.isSele then
            return k
        end
    end
    return nil
...
function ui.addPetData(data, overwritePetId)
    --先清空原有的战宠信息
    ...
    --判断错误情况
	local sel = petNetData.sele
	if overwritePetId then
		sel = overwritePetId
	end
    if sel == nil or sel <= 0 then
        return data
    end
    print("petNetData.sele = "..sel)

    if petdata[sel] == nil then
        return data
    end

    local petTable = {}
    petTable.id = tonumber(sel)
    petTable.pos = 7
    table.insert(data,petTable)
    return data
end
...
function ui.refreshCard()
    ...
    for id,val in pairs(ui.widget.Card) do
        --如果没有数据，置灰
        if petNetData.getData(id) == nil then
            --ui.widget.Card[id].battleMenu:setTouchEnabled(false) 
        ...
        --如果有数据，但是没有被选中
        elseif id ~= petNetData.sele then
            --清理置灰
            ...
            --clearShader(ui.widget.Card[id].battleMenu , true)
