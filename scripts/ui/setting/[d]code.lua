...
function ui.create(_anim)
    ...
    local layer = boardlayer.create(--[[boardlayer.TAB.FEED--]] 50, _anim)
    local board = layer.inner_board
    local board_w = board:getContentSize().width
    local board_h = board:getContentSize().height

    layer.setTitle(i18n.global.midas_get.string)
    ...
    local lbl_feed = lbl.create({kind="ttf", size=18, text="", color=ccc3(0x70, 0x4a, 0x2b),})
    ...
    local lbl_send = lbl.createFont1(18, i18n.global.midas_get.string, ccc3(0x73, 0x3b, 0x05))
    ...
    btn_send:registerScriptTapHandler(function()
        ...
        if not isValidHex(tmp_str) then
            showToast(i18n.global.input_invalid_char.string)
            return
        end
		if #tmp_str ~= 14 then
			showToast(string.format(i18n.global.guild_name_length.string, 14))
            return
		end
        local params = {
            sid = player.sid + 256,
        ...
        addWaitNet()
        netClient:feedback(params, function(__data)
            delWaitNet()
            tbl2string(__data)
            if __data.status ~= 0 then
				if __data.status == 4 or __data.status == 5 then
