...
function ui.create()
    ...
    local lbl_help = lbl.createFont1(18, i18n.global.setting_btn_help.string, ccc3(0x94, 0x62, 0x42))
    ...
    local function showContent(faqObj)
        ...
        local item_title = lbl.createMixFont1(18, faqObj.title, ccc3(0x94, 0x62, 0x42))
        item_title:setPosition(CCPoint(332, 25))
        bar:addChild(item_title)
        -- content
        scroll.addSpace(55)
        if faqObj.describe then
            local parts = string.split(faqObj.describe, '|')
            for ii=1,#parts do
                local item_title = lbl.createMix({font=1, size=18, text=string.trim(parts[ii]), 
