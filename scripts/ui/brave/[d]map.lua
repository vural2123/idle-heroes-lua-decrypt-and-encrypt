...
function ui.create()
    ...
    local function judgeboxstatus(pos)
        print("*****", databrave.stage, rewardlevel[pos])
    ...
    --local bravebox = DHSkeletonAnimation:createWithFile('spinejson/ui/yuanzheng_baoxiang.json')
    ...
    for i=1 , math.min(databrave.stage, cfgbrave[databrave.id].num) do
        ...
        btnStart:registerScriptTapHandler(function()
            if databrave.enemys[i] then
                ...
                addWaitNet()
                net:brave_pull(params, function(__data)
                    delWaitNet()

                    if __data.enemy then
                        databrave.enemys[i] = __data.enemy
                        layer:addChild(require("ui.brave.gateinfo").create(i), 1000)           
                    end
                end)
            end
    ...
    layer:registerScriptHandler(function(event)
        ...
        elseif event == "cleanup" then
            ...
            img.unload(img.packedOthers.spine_ui_yuanzheng_baoxiang)
        end
    end)
