...
function ui.create(stage)
    local layer = CCLayerColor:create(ccc4(0, 0, 0, POPUP_DARK_OPACITY))

    local enemy = databrave.enemys[stage] 
    ...
    local lblBattle = lbl.createFont1(22, i18n.global.brave_btn_battle.string, ccc3(0x73, 0x3b, 0x05))
    ...
    --为保证战宠位置正确，进行一次排序
    table.sort(enemy.camp, function(a,b) return a.pos<b.pos end )
    tbl2string(enemy)
    for i, v in ipairs(enemy.camp) do
        if v and v.pos ~= 7 then
            --local showHero = img.createHeroHead(v.id, v.lv, true, true, v.wake, nil, pet_id)
            local param = {
                id = v.id,
                lv = v.lv,
                showGroup = true,
