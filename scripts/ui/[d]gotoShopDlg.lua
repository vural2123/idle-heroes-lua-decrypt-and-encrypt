...
function storeDialog.show(parent_obj, from_where, body_text)

    if not body_text then
        body_text = i18n.global.ele_hint_no_gem.string
    end
    ...
    local function process_dialog(data)
        if data.selected_btn == 1 then
            ...
            local storelayer = require "ui.shop.main"
            parent_obj:addChild(storelayer.create(), 1000)
