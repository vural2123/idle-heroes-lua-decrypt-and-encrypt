...
function ui.create(heroData, preHero)
    ...
    local titlelbl = lbl.createFont2(24, i18n.global.hero_talen_tips_title.string, ccc3(0xfa, 0xd8, 0x69))
    ...
    --bg:addChild(heroraw)
    ---- lefthero
    --local lheadbg = img.createHeroHead(preHero.id, heroData.lv, true, true, exstar-1)
    --lheadbg:setScale(0.8)
    --lheadbg:setPosition(BG_WIDTH/2-90, 360)
    --bg:addChild(lheadbg)
    ---- righthero
    --local rheadbg = img.createHeroHead(heroData.id, heroData.lv, true, true, exstar)
    --rheadbg:setScale(0.8)
    --rheadbg:setPosition(BG_WIDTH/2+90, 360)
    --bg:addChild(rheadbg)

    -- star
    --local energizeStar = img.createUISprite(img.ui.hero_energize_star)
    --energizeStar:setPosition(BG_WIDTH/2, 360)
    --bg:addChild(energizeStar)
    ...
    local energizeStarLab = lbl.createFont2(26, heroData.wake-4)
    ...
    bg:addChild(lvraw)
    local lvMax = cfgtalen[heroData.wake-4].addMaxLv
    local curLvMax = 250
    if heroData.wake-4 > 1 then
        curLvMax = cfgtalen[heroData.wake-4-1].addMaxLv
    end
    ...
    bg:addChild(infobg2)
    local info2 = lbl.createFont1(16, i18n.global.hero_skill_unlock.string, ccc3(0xeb, 0xaa, 0x5e))
    ...
    skillIconBg1:setPosition(BG_WIDTH/2, 165-16-65)
    bg:addChild(skillIconBg1, 100)
    local skillIcon1 = img.createSkill(6100)
