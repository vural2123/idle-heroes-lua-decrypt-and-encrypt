...
function tips.create(kind, thing, handler)
    ...
    local costCount = math.floor(thing.num/cfgitem[thing.id].equip.count)
    ...
    if thing.num < cfgitem[thing.id].equip.count then
    ...
    progressFg:setPercentage(thing.num / cfgitem[thing.id].equip.count * 100)
    bg:addChild(progressFg)

    local str = string.format("%d/%d", cfgitem[thing.id].equip.count*costCount, cfgitem[thing.id].equip.count)
    ...
    subBtn:registerScriptTapHandler(function()
        ...
        if inputBox.num > 1 then
            ...
            str = string.format("%d/%d", cfgitem[thing.id].equip.count*inputBox.num, cfgitem[thing.id].equip.count)
            label:setString(str)
        end
    end)
