...
function ui.create(content)
    local mbrs = friend.friends.friendsList 
    ...
    local function createItem(_idx)
        ...
        local lblInvite = lbl.createFont1(16, i18n.global.trial_stage_btn_battle.string, ccc3(0x73, 0x3b, 0x05))
        ...
        applyAgreBtn:registerScriptTapHandler(function()
            ...
            param.pos = content.pos
            param.frd = mbrs[_idx].uid
            
            addWaitNet()
            net:island_fight(param, function(__data)
                delWaitNet()
                tbl2string(__data)
                if __data.status == -4 then
                    showToast(i18n.global.airisland_toast_nohelp.string)
                    return
                end
                ...
                require("fight.helper.ccamp").processCamp(video)

                replaceScene(require("fight.hairisland.loading").create(video))
            end)
        end)
