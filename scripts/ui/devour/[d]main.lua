...
function ui.create()
    ...
    shopBg:setPosition(scalep(960+33, 576))
    ...
    btnInfo:setPosition(scalep(750 + 70, 542))
    ...
    local showTitle = lbl.createFont2(22, i18n.global.devour_lef_title.string, ccc3(0xf6, 0xd7, 0x71))
    ...
    local labSmart = lbl.createFont1(18, i18n.global.devour_btn_smart.string, ccc3(0x06, 0x68, 0x0c))
    ...
    --btnPreviewSp:setPreferredSize(CCSize(150, 50))
    --local labPreview = lbl.createFont1(16, i18n.global.ui_decompose_preview.string, ccc3(0x73, 0x3b, 0x05))
    --labPreview:setPosition(btnPreviewSp:getContentSize().width/2, btnPreviewSp:getContentSize().height/2)
    --btnPreviewSp:addChild(labPreview)
    ...
    evolveBg:setPosition(500 - 12, 544)
    ...
    local showEvolveAll = lbl.createFont2(16, "", ccc3(0xff, 0xf7, 0xe5))
    ...
    local labDevour = lbl.createFont1(18, i18n.global.devour_btn.string, ccc3(0x76, 0x25, 0x05))
    ...
    local showHeroNum = lbl.createFont1(18, string.format(i18n.global.devour_rig_title.string, 0, 0), ccc3(0x6f, 0x4c, 0x38))
    ...
    local function createHerolist()
        ...
        for i=1, #tmp do
            if groupType == 0 then
                if cfghero[tmp[i].id].qlt == starType or starType == 0 then
        ...
        for i=1, #herolist do
            ...
            local showLv = lbl.createFont2(15 * 0.8, herolist[i].lv)
    ...
    local function setStarType(kind)
        starType = kind
    end

    local function setGroupType(kind)
        groupType = kind
    end
    ...
    local labFilter = lbl.createFont1(18, i18n.global.selecthero_btn_hero.string, ccc3(0x73, 0x3b, 0x05))
    ...
    local function onSelect(pos)
        for i=1, MAX_NUM do
            if not hids[i] or hids[i] == 0 then
                ...
                showHeros[i] = img.createHeroHead(herolist[pos].id, herolist[pos].lv, true, true, herolist[pos].wake, nil, nil, nil, herolist[pos].hskills)
    ...
    local function unSelect(pos)
        for i=1, #herolist do
            if herolist[i].hid == hids[pos] then
    ...
    local function onTouchEnd(x, y)
        ...
        local pointOnBoard = lefBoard:convertToNodeSpace(ccp(x, y))
        for i=1, MAX_NUM do
    ...
    local function playParticle()
        ...
        --partAnim:addChildFollowSlot("code_particle_01", part1)
        --partAnim:addChildFollowSlot("code_particle_02", part2)
        --partAnim:addChildFollowSlot("code_particle_03", part3)
    end

    local function onhandle()
        local params = {
            sid = player.sid,
            hid = {}
        }
        ...
        addWaitNet()
        net:hero_decompose(params, function(__data)
            delWaitNet()
            ...
            for _, v in ipairs(__data.items) do
                if v.num > 0 then
                    if v.id < 0 then
                        table.insert(reward.equips, { id = -v.id, num = v.num })
            ...
            layer:runAction(CCSequence:createWithTwoActions(CCDelayTime:create(0.35), 
    ...
    btnDevour:registerScriptTapHandler(function()
        ...
        if isHigh then
            local params = {
                title = "",
                text = i18n.global.devour_tips_high.string,
                handle = onhandle,
            }
    ...
    btnSmart:registerScriptTapHandler(function()
        for i, v in ipairs(herolist) do
            if not v.lock and not v.isUsed then
