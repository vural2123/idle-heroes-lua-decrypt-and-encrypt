    ...
    for i=1,3 do
        ...
        if i <= #params.mbrs then
            ...
            for ii=1, 6 do
				...
				if hids[ii] then
					if hids[ii].hpp and hids[ii].hpp <= 0 then
						setShader(showHero, SHADER_GRAY, true)
					end
					local showHpBg = img.createUISprite(img.ui.fight_hp_bg.small)
					showHpBg:setPosition(showHero:boundingBox():getMidX(), showHero:boundingBox():getMinY() - 4)
					showHpBg:setScale(0.55)
					itemBg:addChild(showHpBg)
					...
					showHpFg:setPercentage(hids[ii].hpp or 100)
