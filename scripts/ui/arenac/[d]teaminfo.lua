...
function ui.create(uiParams)
            ...
            if i<=#teaminfo.mbrs then
                ...
                headBtn:registerScriptTapHandler(function()
                    ...
                    if player.uid == teaminfo.leader then
                        if player.uid ~= teaminfo.mbrs[i].uid then
                            layer:addChild((require"ui.arenac.mbrinfo").create(teaminfo.mbrs[i], "clear"), 100)
                        else
                            layer:addChild((require"ui.arenac.mbrinfo").create(teaminfo.mbrs[i], "none"), 100)
                        end
                    else
                        ...
                        if player.uid ~= teaminfo.mbrs[i].uid then
                            ...
                            layer:addChild((require"ui.arenac.mbrinfo").create(teaminfo.mbrs[i], "quit"), 100)
                        end
                    end
                ...
                addBtn:registerScriptTapHandler(function()
                    ...
                    net:gpvp_friendslist(params,function(__data)
                        ...
                        --    return
                        --end
                        layer:addChild(require("ui.arenac.invidefrd").create(__data.friends))
                    end)
    ...
    invitedBtn:registerScriptTapHandler(function()
        ...
        net:gpvp_applylist(params,function(__data)
            delWaitNet()
            tbl2string(__data)
            layer:addChild(require("ui.arenac.invided").create(__data.mbrs))
            --if __data.status < 0 then
            --    showToast("status:" .. __data.status)
            --    return
            --end
