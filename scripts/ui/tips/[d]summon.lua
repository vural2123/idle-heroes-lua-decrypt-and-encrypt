-- 背包召唤的tips

local tips = {}

require "common.func"
local view = require "common.view"
local img = require "res.img"
local lbl = require "res.lbl"
local json = require "res.json"
local audio = require "res.audio"
local cfgequip = require "config.equip"
local cfgitem = require "config.item"
local cfghero = require "config.hero"
local i18n = require "res.i18n"

local TIPS_WIDTH = 360
local TIPS_HEIGHT = 345

-- kind = "equip" | "item"
-- thing = { id, num } 可为equip或item
-- handler = function(thing)
function tips.create(kind, thing, handler)
    local layer = CCLayer:create()

    local price
    if kind == "items" then
        price = cfgitem[thing.id].recoveryPrice
    end

    -- tips背景
    local bg = img.createUI9Sprite(img.ui.tips_bg)
    bg:setPreferredSize(CCSize(TIPS_WIDTH, TIPS_HEIGHT))
    bg:setScale(view.minScale)
    bg:setPosition(view.physical.w/2, view.physical.h/2)
    layer:addChild(bg)

    -- name
    local nameText, nameColor
    if kind == "items" then
        nameText = i18n.item[thing.id].name
        nameColor = lbl.qualityColors[cfgitem[thing.id].qlt]
    else
		nameText = i18n.hero[thing.id].name
		nameColor = lbl.qualityColors[cfghero[thing.id].qlt or 1]
	end
    local name = lbl.createMix({
        font = 1, size = 18, text = nameText, color = nameColor,
        width = LABEL_WIDTH, align = kCCTextAlignmentLeft,
    })
    name:setPosition(TIPS_WIDTH/2, TIPS_HEIGHT - 30)
    bg:addChild(name)

    -- icon
    local icon
    if kind == "items" then
        icon = img.createItem(thing.id)
    else
		icon = img.createHeroHead(thing.id, 1, true, true)
	end
    icon:setPosition(TIPS_WIDTH/2, TIPS_HEIGHT - 97)
    bg:addChild(icon)

    -- sub button
    local subBtn0 = img.createUISprite(img.ui.tips_sell_sub)
    local subBtn = SpineMenuItem:create(json.ui.button, subBtn0)
    subBtn:setPosition(TIPS_WIDTH/2 - 82, TIPS_HEIGHT - 206)
    local subMenu = CCMenu:createWithItem(subBtn)
    subMenu:setPosition(0, 0)
    bg:addChild(subMenu)

    -- add button
    local addBtn0 = img.createUISprite(img.ui.tips_sell_add)
    local addBtn = SpineMenuItem:create(json.ui.button, addBtn0)
    addBtn:setPosition(TIPS_WIDTH/2 + 82, TIPS_HEIGHT - 206)
    local addMenu = CCMenu:createWithItem(addBtn)
    addMenu:setPosition(0, 0)
    bg:addChild(addMenu)

	local costCount
	if kind == "items" then
		costCount = math.floor(thing.num/cfgitem[thing.id].heroCost.count)
	else
		costCount = thing.num
	end

    -- input box
    local inputBoxBg = img.createLogin9Sprite(img.login.input_border)
    local inputBox = CCEditBox:create(CCSize(90*view.minScale, 35*view.minScale), inputBoxBg)
    inputBox:setInputMode(kEditBoxInputModeNumeric)
    inputBox:setReturnType(kKeyboardReturnTypeDone)
    inputBox:setFont("", 14*view.minScale)
    inputBox:setFontColor(ccc3(0x42, 0x24, 0x06))
    inputBox:setText(tostring(costCount))
    inputBox:setPosition(scalep(480, 256))
    inputBox.num = costCount
    layer:addChild(inputBox, 1)

    -- coin
    --local coinBg = img.createUI9Sprite(img.ui.tips_sell_coin_bg)
    --coinBg:setPreferredSize(CCSize(202, 33))
    --coinBg:setPosition(TIPS_WIDTH/2, TIPS_HEIGHT - 226)
    --bg:addChild(coinBg)
    --local coinIcon = img.createItemIcon2(ITEM_ID_COIN)
    --coinIcon:setPosition(110, TIPS_HEIGHT - 226)
    --bg:addChild(coinIcon)
    --local coinLabel = lbl.createFont2(18, tostring(price * inputBox.num))
    --coinLabel:setPosition(TIPS_WIDTH/2, TIPS_HEIGHT - 226)
    --bg:addChild(coinLabel)

    -- summon button
    local summonBtn0 = img.createLogin9Sprite(img.login.button_9_small_gold)
    summonBtn0:setPreferredSize(CCSize(138, 47))
    local summonBtn = SpineMenuItem:create(json.ui.button, summonBtn0)
    local summonBtnSize = summonBtn:getContentSize()
    summonBtn:setPosition(TIPS_WIDTH/2, TIPS_HEIGHT - 289)
    local summonLabel = lbl.createFont1(18, i18n.global.tips_summon.string, ccc3(0x73, 0x3b, 0x05))
    summonLabel:setPosition(summonBtnSize.width/2, summonBtnSize.height/2)
    summonBtn0:addChild(summonLabel)
    local summonMenu = CCMenu:createWithItem(summonBtn)
    summonMenu:setPosition(0, 0)
    bg:addChild(summonMenu)
    summonBtn:registerScriptTapHandler(function()
        audio.play(audio.button)
        if handler then
			local harg = { id = thing.id }
			if kind == "items" then
				harg.num = inputBox.num*cfgitem[thing.id].heroCost.count
			else
				harg.num = inputBox.num
			end
            handler(harg)
        end
    end)

    local progressBg = img.createUISprite(img.ui.bag_heropiece_progr)
    progressBg:setPosition(TIPS_WIDTH/2, TIPS_HEIGHT - 150)
    bg:addChild(progressBg)

    local progressFgSprite = nil
	local minreq = 1
	if kind == "items" then
		minreq = cfgitem[thing.id].heroCost.count
	end
    if thing.num < minreq then
        progressFgSprite = img.createUISprite(img.ui.bag_heropiece_progr_0)
    else
        progressFgSprite = img.createUISprite(img.ui.bag_heropiece_progr_1)
    end

   
    local progressFg = createProgressBar(progressFgSprite) 
    progressFg:setPosition(TIPS_WIDTH/2, TIPS_HEIGHT - 150)
    progressFg:setPercentage(thing.num / minreq * 100)
    bg:addChild(progressFg)

    local str = string.format("%d/%d", minreq*costCount, minreq)
    local label = lbl.createFont2(14, str, ccc3(255, 246, 223))
    label:setPosition(TIPS_WIDTH/2, TIPS_HEIGHT - 150)
    bg:addChild(label)

    subBtn:registerScriptTapHandler(function()
        audio.play(audio.button)
        if inputBox.num > 1 then
            inputBox.num = inputBox.num - 1
            inputBox:setText(tostring(inputBox.num))
            str = string.format("%d/%d", minreq*inputBox.num, minreq)
            label:setString(str)
        end
    end)

    addBtn:registerScriptTapHandler(function()
        audio.play(audio.button)
        if inputBox.num < costCount then
            inputBox.num = inputBox.num + 1
            inputBox:setText(tostring(inputBox.num))
            str = string.format("%d/%d", minreq*inputBox.num, minreq)
            label:setString(str)
        end
    end)

    inputBox:registerScriptEditBoxHandler(function(event)
        if event == "ended" then
            local num = checkint(string.trim(inputBox:getText()))
            if num < 1 then
                num = 1
            elseif num > costCount then
                num = costCount
            end
            inputBox.num = num
            inputBox:setText(tostring(num))
            str = string.format("%d/%d", minreq*inputBox.num, minreq)
            label:setString(str)
        elseif event == "changed" then 
        end
    end)

    -- 点击空白区域的回调
    local clickBlankHandler
    function layer.setClickBlankHandler(handler)
        clickBlankHandler = handler
    end

    local beginx, beginy
    local function onTouch(eventType, x, y)
        if eventType == "began" then   
            beginx, beginy = x, y
            return true
        elseif eventType == "moved" then
            return 
        else
            if not bg:boundingBox():containsPoint(ccp(x, y)) 
                and not bg:boundingBox():containsPoint(ccp(beginx, beginy)) then
                layer.onAndroidBack()
            end
            return
        end
    end

    addBackEvent(layer)

    function layer.onAndroidBack()
        if clickBlankHandler then
            clickBlankHandler()
        else
            layer:removeFromParent()
        end
    end

    layer:registerScriptHandler(function(event)
        if event == "enter" then
            layer.notifyParentLock()
        elseif event == "exit" then
            layer.notifyParentUnlock()
        end
    end)

    layer:registerScriptTouchHandler(onTouch)
    layer:setTouchEnabled(true)
    layer:setTouchSwallowEnabled(true)

    return layer
end

return tips
