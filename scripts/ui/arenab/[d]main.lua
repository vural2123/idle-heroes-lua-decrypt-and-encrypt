...
function ui.create(uiParams)
    ...
    btnBack:setPosition(35-40, 546-18)
    ...
    btnInfo:registerScriptTapHandler(function()
        audio.play(audio.button)
        local str = i18n.arena[5].infoTitle1 .. ":::" .. string.gsub(i18n.arena[5].info1, ";", "|||")
        str = str .. "###" .. i18n.arena[5].infoTitle2 .. ":::" .. string.gsub(i18n.arena[5].info2, ";", "|||")
    ...
    local showTitle = lbl.createFont3(22, i18n.arena[5].name)
    ...
    btnReward:registerScriptTapHandler(function()
        audio.play(audio.button)
        layer:addChild(require("ui.arenab.rewards").create())
    end)
    ...
    btnRecord:registerScriptTapHandler(function()
        audio.play(audio.button)
        layer:addChild(require("ui.arenab.records").create())
    end)
    ...
    btnDefen:registerScriptTapHandler(function()
        ...
        audio.play(audio.button)
        layer:addChild(require("ui.selecthero.main").create({type = "ArenabDef"})) 
    end)
    ...
    btnBattle:registerScriptTapHandler(function()
        audio.play(audio.button)
        layer:addChild(require("ui.arenab.pickRival").create())
    end)
	
	if arenaData.fight and arenaData.fight > 0 then
		require("ui.custom").setButtonEnabled(btnBattle, false)
	end
    ...
    local function loadRank(ranks)
        ...
        for i, v in ipairs(ranks) do
            ...
            --showHead:setPosition(122, 43)
            --playerBg:addChild(showHead)
    ...
    local function pullRank()
        if (not arenaData.members) or arenaData.rank <= 50 or (arenaData.rank >= 999999 and arenaData.score > 0) then
            local params = {
                sid = player.sid,
                id = 5,
            }
            ...
            net:pvp_rank(params, function(__data)
                ...
                if __data.status == -1 then
                    loadRank(arenaData.members or {})
                else
                    arenaData.members = __data.members or {}
                    loadRank(__data.members or {})
                end

                for i=1, #arenaData.members do
                    if arenaData.members[i].uid == player.uid then
                        arenaData.rank = i
                        if arenaData.trank and arenaData.trank > arenaData.rank then
