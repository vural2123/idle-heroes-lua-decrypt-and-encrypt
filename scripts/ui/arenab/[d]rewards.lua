...
local function createSeasonRewards()
    ...
    scroll:setViewSize(CCSize(682, THING_SIZE))
    ...
    for i=1, #cfgSeasons do
        if cfgSeasons[i].arenaId == 5 then
    ...
    scroll:setContentOffset(ccp(0, THING_SIZE -  scroll:getContentSize().height))    
...
function ui.create()
    ...
    showRewardBg:setPreferredSize(CCSize(682, THING_SIZE + 1))
    showRewardBg:setAnchorPoint(ccp(0, 0))
    showRewardBg:setPosition(22, 27)
    board:addChild(showRewardBg)

    showRewardBg:addChild(createSeasonRewards(showRewardBg), 1, 1)
